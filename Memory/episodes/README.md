...
Bounded execution records for AI Informer Builder, created when `advanced-episode-flow` is active.

**You MUST read this file before creating or updating any episode.**

## Keep Episode Files Concise

The episode file is the durable, decision-relevant record — **not** the place where full execution context flows. Planning, execution, and evaluation details are routed between the main agent and its subagents (`episode-planner`, `{{EPISODE_EXECUTOR_NAME}}`, `episode-evaluator`) via concise prompts and return values, not by dumping raw transcripts here. Put only what the agent itself will need when reading this episode back later.
...
An episode file with no `status` field is mid-flight and not authoritative. The planner creates the file without `status`; the executor appends concise Actions Taken and Key Evidence during execution; the main agent writes final `status` at the end of the heartbeat when the outcome is decided.
